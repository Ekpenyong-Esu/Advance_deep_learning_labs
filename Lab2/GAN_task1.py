...
# Training
def cGANTraining(G, D, loss_fn, train_loader, G_solver, D_solver, device,D_out):
    G.train()
    D.train()
    ...
    for it, (X_real, F) in enumerate(t):
        # Prepare real data
...
########################### Main #######################################
def run_training(use_logistic_loss=False):
    wandb_log = True
    ...
    if use_logistic_loss:
        ...
        loss_fn = nn.BCELoss()
        def D_out(x):
            return torch.sigmoid(D(x)) 

    #loss_fn = nn.BCEWithLogitsLoss()
    

    if wandb_log: 
        wandb.init(project="advanced-ai-lab2")
        # Log hyperparameters
        wandb.config.update({
            "batch_size": mb_size,
            "Z_dim": Z_dim,
    ...
    for epoch in range(epochs):
        ...
        print(D_out)
        G, D, G_loss_avg, D_loss_avg = cGANTraining(G, D, loss_fn, train_loader, G_solver, D_solver, device,D_out)

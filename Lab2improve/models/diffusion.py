...
FIXES Applied
-------------
  1. sinusoidal_embedding  — corrected frequency denominator (/ half, not / max(half-1,1))
  2. ConditionalDenoiser   — added LayerNorm after every hidden layer for training stability
  3. GaussianDiffusion.sample — added .clamp(-1, 1) guard on final output
  4. Data normalisation note — sample() expects [-1, 1] input; caller must denorm with (x+1)/2
...
def sinusoidal_embedding(timesteps: torch.Tensor, dim: int) -> torch.Tensor:
    ...
    FIX: denominator changed from max(half-1, 1) → half to match the
         standard Transformer / DDPM formulation exactly.
    """
    half  = dim // 2
    freqs = torch.exp(
        -math.log(10_000)
        * torch.arange(half, dtype=torch.float32, device=timesteps.device)
        / half                                          # ← FIXED (was max(half-1,1))
    )
...
class ConditionalDenoiser(nn.Module):
    ...
    FIX: Added LayerNorm after every hidden layer.  Without normalisation the
         activations vary wildly across the 1000 noise levels, making training
         unstable and preventing the network from learning to denoise.
    """
    ...
    def __init__(self, x_dim: int = 784, t_emb_dim: int = 128,
        ...
        self.net = nn.Sequential(
            ...
            nn.LayerNorm(h_dim),                        # ← ADDED
            nn.SiLU(),
            nn.Linear(h_dim, h_dim),
            nn.LayerNorm(h_dim),                        # ← ADDED
            nn.SiLU(),
            nn.Linear(h_dim, h_dim // 2),
            nn.LayerNorm(h_dim // 2),                   # ← ADDED
            nn.SiLU(),
...
class GaussianDiffusion(nn.Module):
    ...
    Notes
    -----
    • Training data must be normalised to [-1, 1]  (not [0, 1]).
      This aligns the clean-image range with the N(0, I) noise prior,
      which is essential for the diffusion process to work correctly.

    • After sampling, convert back to [0, 1] with:
          images = (samples.clamp(-1, 1) + 1) / 2
      Do NOT use per-batch min/max rescaling — it masks failed samples.
    """
    ...
    def p_losses(self, x0: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        ...
        Parameters
        ----------
        x0 : (B, x_dim) clean images in [-1, 1]   ← must be [-1, 1], not [0, 1]
    ...
    def sample(self, y: torch.Tensor, x_dim: int = 784) -> torch.Tensor:
        ...
        Returns
        -------
        x : (B, x_dim) generated images in [-1, 1]

        Caller should convert to [0, 1] for display:
            images = (samples.clamp(-1, 1) + 1) / 2

        FIX: final output is clamped to [-1, 1] to prevent drift.
             Do NOT use per-batch min/max rescaling in the caller.
        """
        ...
        return x.clamp(-1.0, 1.0)                      # ← FIXED (was returned raw)